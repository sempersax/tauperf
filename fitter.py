...
train_1p0n, test_1p0n = model_selection.train_test_split(
    images_1p0n, test_size=0.2, random_state=42)
         
train_1p1n, test_1p1n = model_selection.train_test_split(
    images_1p1n, test_size=0.2, random_state=42)

train_1p2n, test_1p2n = model_selection.train_test_split(
    images_1p2n, test_size=0.2, random_state=42)

headers = ["sample", "Total", "Training", "Testing"]
sample_size_table = [
    ['1p0n', len(images_1p0n), len(train_1p0n), len(test_1p0n)],
    ['1p1n', len(images_1p1n), len(train_1p1n), len(test_1p1n)],
    ['1p2n', len(images_1p2n), len(train_1p2n), len(test_1p2n)],
]
...
model_pi0_filename = 'cache/crackpot_pi0_new.h5'
...
model_twopi0_filename = 'cache/crackpot_twopi0_new.h5'
model_twopi0 = dense_merged_model(train_twopi0)
load_or_fit(
    model_twopi0,
    train_twopi0, y_train_twopi0,
    test_twopi0, y_test_twopi0,
...
log.info('compute 1p1n vs 1p0n classfier scores')
...
pred_pi0 = pred_pi0.reshape((y_pred_pi0.shape[0],))
pred_twopi0 = pred_twopi0.reshape((y_pred_twopi0.shape[0],))


# 1p1n vs 1p0n - pt 
eff_pi0_pt_1p1n = get_eff(
    test_1p1n['pt'], pred_pi0[y_true == 1], 
    scale=0.001, color='red', name='1p1n')

eff_pi0_pt_1p0n = get_eff(
    test_1p0n['pt'], y_pred_pi0[y_true == 0] == 0, 
...
eff_pi0_eta_1p1n = get_eff(
    test_1p1n['eta'], pred_pi0[y_true == 1], 
    binning=(10, -1.1, 1.1), color='red', name='1p1n')

eff_pi0_eta_1p0n = get_eff(
    test_1p0n['eta'], pred_pi0[y_true == 0] == 0, 
...
eff_pi0_mu_1p1n = get_eff(
    test_1p1n['mu'], pred_pi0[y_true == 1], 
    binning=(10, 0, 40), color='red', name='1p1n')

eff_pi0_mu_1p0n = get_eff(
    test_1p0n['mu'], pred_pi0[y_true == 0] == 0, 
...
eff_twopi0_pt_1p1n = get_eff(
    test_1p1n['pt'], pred_twopi0[y_true == 1] == 0, 
    scale=0.001, color='red', name='1p1n')

eff_twopi0_pt_1p2n = get_eff(
    test_1p2n['pt'], pred_twopi0[y_true == 2], 
...
eff_twopi0_eta_1p1n = get_eff(
    test_1p1n['eta'], pred_twopi0[y_true == 1] == 0, 
    binning=(10, -1.1, 1.1), color='red', name='1p1n')

eff_twopi0_eta_1p2n = get_eff(
    test_1p2n['eta'], pred_twopi0[y_true == 2], 
...
eff_twopi0_mu_1p1n = get_eff(
    test_1p1n['mu'], pred_twopi0[y_true == 1] == 0, 
    binning=(10, 0, 40), color='red', name='1p1n')

eff_twopi0_mu_1p2n = get_eff(
    test_1p2n['mu'], pred_twopi0[y_true == 2], 
