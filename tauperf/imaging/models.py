...
##################################################################################
# This is a model built by Torrey Saxton, under the direction of Quentin Buat.
# The intent of this model is to take the data that has been previously used, and
# reformatted and create a convulutional 2d neural network.  Taking advantage of
# the stride and dilation functions built-in to Keras, the intent is to Max Pool 
# each layer of the calorimeter, and then concatenate each layer's network, thus
# creating a "RGB-Like" network
##################################################################################
def RGB_like_merged_model(data, n_classes=3, final_activation='softmax'):
    """
    """
    ...
    s1_x = Reshape((s1_shape[0], s1_shape[1], 1))(s1_input)
    log.info('\t s1 input shape   = {0}'.format(s1_x._keras_shape))
    s1_x = Conv2D(64, (6, 2), dilation_rate=(2,2),padding='same', activation='relu')(s1_x)
    log.info('\t s1 convolu shape = {0}'.format(s1_x._keras_shape))
    s1_x = MaxPooling2D((1, 8))(s1_x)
    log.info('\t s1 maxpool shape = {0}'.format(s1_x._keras_shape))
    s1_x = Dropout(0.2)(s1_x)
    s1_out = (s1_x)
    log.info('\t s1 final shape   = {0}'.format(s1_out._keras_shape))
